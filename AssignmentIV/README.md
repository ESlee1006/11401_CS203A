...
|----------------|--------------------------------------------------------------------------|----------------------|
| 10             | 5, 2, 4, 9, 1, 6, 8, 3, 9, 8, 6, 2, 9, 7, 0, 6, 3, 9, 6, 4               |範圍介於0 ~ m-1, 無規律, index重複高, collision發生機率大|
...
| Table Size (m) |                             Index Sequence                           |     Observation      |
|----------------|----------------------------------------------------------------------|----------------------|
|10              | 1, 7, 8, 8, 0, 5, 9, 4, 7, 0                                         |範圍介於0 ~ m-1, index重複較高, collision發生頻率較高|
|11              | 4, 0, 3, 10, 9, 5, 4, 9, 1, 8                                        |範圍介於0 ~ m-1, index重複較低, collision發生頻率較小|
|37              | 21, 14, 0, 14, 14, 0, 30, 5, 7, 18                                   |範圍介於0 ~ m-1, index重較高, collision發生頻率較高|
...
## Analysis ##
- 直接用 (hash % m) 且 m 非 2 的冪時，取模會導致偏差，因此即使哈希函數本身不錯，取模步驟也會引入不均勻
- 用 XOR (key >> 16) 能把高位資訊帶入低位，改善某些有序輸入的混合，但不能消除由取模引起的偏差
- FNV 對短字串與小資料集通常表現良好，但當哈希空間被小 m 強制壓縮時（取模），仍會產生碰撞。短且相似的字串更容易在小模數下碰撞

## Reglection ##
- Knuth 的乘法常數與 XOR 技巧能有效混合整數位元，但若最後用 % m（m 非 2 的冪），會掩蓋哈希本身的優勢；最佳實務是配合表大小選取合適的縮減方法（例如 multiply-shift）。
- FNV 是輕量且快速的字串哈希，對短字串常常足夠，但為了更佳的分布與抗碰撞，使用 64-bit 或更先進的非加密哈希（Murmur/xxHash）更佳。
